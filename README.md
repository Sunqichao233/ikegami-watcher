...
> 🔒 **本文件严禁写入教習生番号与密码。**
> 当前方案（油猴脚本）依附于浏览器中已登录的会话，**不需要保存任何凭据**。
...
→ 因此最优解是**两者并用**：官方邮件为主，油猴脚本作兜底与主动扫描。
...
| 纯 HTTP 请求（requests 等） | ⚠️ 可行但已弃用 | 无 CSRF token，技术上成立；但会踢掉本人登录，且需明文保存密码 |
| **油猴脚本 + 点击「次の週」** | ✅ **采用** | 跑在已登录页面内，零会话冲突，无需保存凭据 |

> **纯 HTTP 方案已移除实现。** §5.9 确认无 CSRF token 后它一度可行，
> 但相比油猴没有实际优势：两者都要电脑开着，而它额外引入
> 単一セッション冲突与凭据落盘风险。所需的全部接口信息保留在 §5.6–§5.9b，
> 日后若要重做可直接依据那几节。
...
- ❌ 不必追求高频（邮件已覆盖实时性）
- ✅ 改为**盯紧特定时段**（⚙ 设置里精确到星期 × 时限）
- ✅ 人在电脑前时开着即可，`intervalMs` 60 秒足够
...
- [x] ~~找出「次の週」按钮的选择器~~ ✅ 脚本自动探测（onclick → 文本 → id/class → 表单兜底）

**实现**
- [x] ~~编写油猴监控脚本~~ ✅ `ikegami-watcher.user.js`（§8.4）
- [ ] 自动预约：需先抓到确认画面上「予約する」的最终提交请求
...
| 2026-07-31 | 抓到 **confirm 请求**（§5.9b）。**参数映射完全解开**：`data-yoyaku`→`dateInformationType`、`data-zigen`→`infoPeriodNumber`、`data-time`→`selectTime`。确认 nextWeek 与 confirm 共用同一参数模板。实现 `monitor.py`（后已移除，见 2026-08-17） |
...
| 2026-08-17 | **收敛为单一油猴方案**。移除 GitHub Actions 与 `monitor.py` / `requirements.txt` —— 无云端调度后，本地跑 Python 相比油猴无优势，反而引入単一セッション冲突与凭据落盘风险。接口分析（§5.6–§5.9b）完整保留，日后若要重做纯 HTTP 方案可直接依据 |
